...
import { mapPlaces } from "./src/client-preferences.js";
import { normalizeLocale } from "./views/shared/i18n.js";

const server = new MCPServer({
  name: "xweather-template", title: "Xweather Template", version: "0.3.0",
...
async function mapResult(places: ReturnType<typeof mapPlaces>, layers: z.infer<typeof layerSchema>[], zoom: number, locale?: string) {
  const { location, locations, locationSource } = places;
  ...
  const data = mapSchema.parse({ mode: dataMode(), location: resolved, locationSource, locale: normalizeLocale(locale), layers: [...new Set(layers)], zoom,
...
export const showWeatherMap = server.tool({
  ...
  description:"Open a map-only Storm Explorer with radar, lightning, storm cells, place pins and playback. Omit location to use the client's current approximate location; Dallas is the fallback when no client location is supplied. An explicit location takes priority. Storm cells have US coverage.",
  inputSchema:z.object({location:querySchema.optional(),locations:z.array(querySchema).max(5).default([]),layers:z.array(layerSchema).max(9).default(["radar","lightning-strikes","stormcells","alerts"]),zoom:z.number().min(2).max(12).default(6.5)}),
...
}, async ({location,locations,layers,zoom},ctx) => {try{const user=ctx.client.user();return await mapResult(mapPlaces(location,locations,user?.location),layers,zoom,user?.locale);}catch(error){return toolError(error);}});
export const showWeatherDashboard = server.tool({
  name:"show-weather-dashboard",title:"Weather sites on a map",
  description:"Compare up to five places as pins in the Storm Explorer. Omit locations to show the client's current approximate location, or Dallas/Arlington/Plano when no client location is supplied. Selecting a pin opens its weather and official alerts over the map.",
  inputSchema:z.object({locations:z.array(querySchema).min(1).max(5).optional()}),
...
},async ({locations},ctx) => {try{const user=ctx.client.user();return await mapResult(mapPlaces(undefined,locations,user?.location,true),["radar","lightning-strikes","stormcells","alerts"],6.5,user?.locale);}catch(error){return toolError(error);}});
